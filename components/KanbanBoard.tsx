          ...
          <div className="mt-4 flex items-center justify-between">
            <div className="flex space-x-4">
              ...
              >
                ...
                <span>
                  {idea.votes?.length || 0} {hasVoted ? "Voted" : "Vote"}
                </span>
              </button>
...
const KanbanBoard: React.FC<KanbanBoardProps> = ({
    ...
    <div className="flex flex-col md:flex-row h-full overflow-hidden">
      {(Object.keys(columns) as ColumnType[]).map((status) => (
        ...
        >
          ...
          <div
            className={`p-4 rounded-lg h-full border-2 overflow-y-auto ${getColumnStyle(
              status
            )}`}
          >
            {columns[status].map((idea) => (
              <div
                ...
                draggable={isAdmin}
                onDragStart={
                  isAdmin ? (e) => handleDragStart(e, idea) : undefined
                }
              >
                {renderIdea(idea)}
              </div>
  ...
  const renderListView = () => (
    <div className="space-y-4">{ideas.map((idea) => renderIdea(idea))}</div>
  );
